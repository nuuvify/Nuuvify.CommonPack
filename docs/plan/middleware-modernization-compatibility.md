...
| 2026-08-22 | Em andamento -> Em andamento | Corrigida regressão de sintaxe em `TemplateDotnetApi/CwsRepository`; diretivas `using` alinhadas ao padrão; suítes completas de API e Worker passaram |
| 2026-08-23 | Em andamento -> Em andamento | Etapa 3 de API key concluída com correção da claim canônica e compatibilidade dupla; README do pacote Security atualizado e testes focados da solução Nuuvify reexecutados |
| 2026-08-23 | Em andamento -> Em andamento | Etapa 4 fechada após corrigir a composição de `AddCanonicalValidation`; Etapa 5 iniciada com regressões dos proxies dotenv de API e Worker, sem mutação do ambiente do processo |
| 2026-08-23 | Em andamento -> Em andamento | `CredentialApiOptions` passou a aceitar `IConfiguration` diretamente; referências de observabilidade e construtor legado de `CwsRepositoryExample` foram alinhados sem remover contratos 2.x |
| 2026-08-23 | Em andamento -> Em andamento | `CwsRepositoryExample` recebeu construtor canônico com `IConfiguration`; leituras de URLs e cache passaram pelo resolvedor nativo com fallback legado; construtor anterior foi preservado |
| 2026-08-23 | Em andamento -> Em andamento | `ServiceBusPermissionTester` recebeu construtor nativo com `IConfiguration` e mantém o adaptador legado; teste focado do Worker compilou e iniciou sem erros após desambiguar o caso nulo |
| 2026-08-23 | Em andamento -> Em andamento | As duas últimas leituras efetivas de `IConfigurationCustom` em `CwsRepositoryExample` foram migradas para o resolvedor nativo; suíte completa de `Infra.Http` passou com 26 testes e zero falhas |
| 2026-08-23 | Em andamento -> Em andamento | `Infra.Base` foi alinhado à abstração de observabilidade já usada pelo construtor canônico de `BaseRepository`; suíte de testes compilou e iniciou, mantendo apenas avisos dos construtores legados |
