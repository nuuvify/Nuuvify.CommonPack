    ...
    internal static class Db2ModelBuilderExtensions
    {
            ...
            Action<AutoHistoryOptions> configure) where TAutoHistory : AutoHistory
        {
            ...
            modelBuilder.Entity<TAutoHistory>(b =>
            {
                ...
                b.Property(c => c.Id).IsRequired().HasColumnName("ID").HasColumnType($"CHAR(36)");
                b.Property(c => c.RowId).IsRequired().HasColumnName("ROW_ID").HasColumnType($"CHAR({options.RowIdMaxLength})");
                b.Property(c => c.CorrelationId).HasColumnName("CORRELATION_ID").HasColumnType($"CHAR({options.CorrelationIdMaxLength})");
                b.Property(c => c.TableName).IsRequired().HasColumnName("TABLE_NAME").HasColumnType($"CHAR({options.TableMaxLength})");
                ...
                b.Property(c => c.Kind)
                    .IsRequired()
                    .HasColumnName("KIND")
                    .HasColumnType($"CHAR({options.KindMaxLength})")
                    .HasConversion(v => v.ToString(),
                                   v => (EntityState)Enum.Parse(typeof(EntityState), v));

                b.Property(c => c.Created).IsRequired().HasColumnName("CREATED");
                b.Property(c => c.Username).HasColumnName("USERNAME").HasColumnType($"CHAR(36)");
                b.Property(c => c.PersistInDatabase).HasColumnName("PERSIST").HasColumnType($"CHAR(10)");
